...
class ServerConnectionService {
  ...
  private async _buildAdaptivePortList(primaryPort?: string): Promise<string[]> {
    ...
    // 2. Saved port from storage (read through encryptedStorage — KEYS.PORT is sensitive)
    const savedPort = await encryptedStorage.getItem(KEYS.PORT).catch(() => null);
    add(savedPort);
  ...
  // ── Persist — all sensitive fields go through encryptedStorage ─
  ...
  // FIX 1A — ensure storage is loaded into memory once (via encryptedStorage)
  ...
  async loadKnownGood(): Promise<void> {
    try {
      // Load from encryptedStorage (IP:port pairs are sensitive)
      const raw = await encryptedStorage.getItem(KNOWN_GOOD_KEY);
  ...
  async saveKnownGood(ip: string, port: string): Promise<void> {
    ...
    // Stored through encryptedStorage — IP:port list is sensitive
    await encryptedStorage.setItem(KNOWN_GOOD_KEY, JSON.stringify(this._knownGoodIPs)).catch(() => {});
  }
        ...
        try {
          ...
          if (res.status < 500) {
            ...
            if (tryPort !== this._port && tryPort !== port) {
              this._port = tryPort;
              await encryptedStorage.setItem(KEYS.PORT, tryPort).catch(() => {});
            }
  ...
  async pair(ip: string, port: string, pairingCode: string = '', skipPing = false, appSig = ''): Promise<ConnResult & { token?: string }> {
    ...
    try {
      ...
      if (res.ok && (data.sessionToken || data.token)) {
        ...
        this._appSig = data.appSig || this._appSig;
        this._ok     = true;
        await this._save();
        ...
        }).then(_r => { /* silent verify — status not logged in production */ }).catch(() => {});
  ...
  async saveManual(ip: string, port: string): Promise<void> {
    ...
    // Persist to encrypted storage only — do NOT call _notify() here
  ...
  async connectManual(ip: string, port: string): Promise<ConnResult> {
    ...
    // Persist the actually-working port immediately (encrypted)
    await encryptedStorage.multiSet([
      [KEYS.IP,   ip],
  ...
  async fetchWithAuth(url: string, options: RequestInit = {}): Promise<Response> {
    const headers: Record<string, string> = {
      ...
      ...(this._appSig  ? { 'X-Butler-App-Sig': this._appSig } : {}),
  ...
  async disconnect(): Promise<void> {
    this._ok    = false;
    this._token = '';
    // Remove through encryptedStorage (handles both plain and encrypted residue)
    await encryptedStorage.removeItem(KEYS.TOKEN).catch(() => {});
    await encryptedStorage.removeItem(KEYS.PAIRED_AT).catch(() => {});
    this._notify();
  }

  async clearAll(): Promise<void> {
    this._ip = this._port = this._device = this._token = this._appSig = '';
    this._ok = false;
    // Wipe all sensitive keys through encryptedStorage
    await Promise.all(
      Object.values(KEYS).map(k => encryptedStorage.removeItem(k).catch(() => {})),
    );
    // Also wipe the derived-key vault so a fresh pair generates a fresh key
    try {
      const SecureStore = require('expo-secure-store');
      await SecureStore.deleteItemAsync('butler_derived_key_v1').catch(() => {});
      await SecureStore.deleteItemAsync('butler_derived_device_id_v1').catch(() => {});
    } catch {}
    // Wipe known-good list
    await AsyncStorage.removeItem(KNOWN_GOOD_KEY).catch(() => {});
    this._notify();
  }
